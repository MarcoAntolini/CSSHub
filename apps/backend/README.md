...
3. [`apps/backend/vercel.json`](vercel.json) runs `npm ci` from the monorepo root, **builds and vendors `@csshub/shared`** (workspace symlinks are not bundled by Vercel; install copies `packages/shared` into `node_modules/@csshub/shared`), sets `framework: null` (API-only, no static `public` output), and includes `lib/**` plus the vendored package in each function bundle
